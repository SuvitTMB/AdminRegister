...
function loadData(x){
  ...
  .get().then((snapshot)=> {
    snapshot.forEach(doc=> {
      ...
      var sEmpMember = "";
      if(doc.data().EmpMember==1) { sEmpMember = "สมาชิก"; } else { sEmpMember = "-"; }
        //if(doc.data().StatusRegister==0) { sEmpMember = "-"; } else { sEmpMember = "x"; }
      if(doc.data().TimegetBox!="") { xTimegetBox="รับแล้ว"; } else { xTimegetBox="-"; }
      //} else {
      //  sEmpMember = "-";
      //  xTimegetBox="-";
      //}
      ...
      if(doc.data().StatusRegister==1) {
        ...
        sStatusRegister = "รอยืนยัน"
      }

/*
      var sTimeStamp = "";
      if(doc.data().PreDateTime!="") {
        if(doc.data().DateTime=="") {
          sTimeStamp = "รอยืนยัน";
        } else {
          sTimeStamp = "ลงทะเบียน";
        }
      } else {
        sTimeStamp = "ลงทะเบียน";
      }
*/
      ...
      dataSet = [doc.data().LineName, doc.data().EmpID, doc.data().EmpName, sPreRegister, sStatusRegister, sEmpMember,xTimegetBox,doc.data().EmpRH, doc.data().EmpSize, doc.data().DateTime,  doc.id, i];
